...
st.write("""donde:""")
- st.latex(N):st.write(""" Cantidad total de paneles en el arreglo.""")
- st.latex(G): st.write("""Irradiancia global incidente en forma normal a los módulos fotovoltaicos, en W/m2.
...
- st.latex(G_{std}):st.write(""" Irradiancia estándar, en W/m2. Es un valor de irradiancia que utilizan los fabricantes
de los módulos para referenciar ciertas características técnicas. Normalmente """)
- st.latex(G_{std} = 1000 [W/m2]).
- st.latex(T_r): st.write("""Temperatura de referencia, en Celsius. Es una temperatura utilizada por los
...
- st.latex(P_{pico}): st.write("""Potencia pico de cada módulo, en Watt. Se interpreta como la potencia eléctrica
que entrega un módulo cuando""") st.latex(G) st.write("""coincide con""") st.latex(G_{std}) 
st.write("""y cuando""") st.latex(T_c) st.write("""coincide con""") st.latex(T_r)st.write(""", en
...
- st.latex(kp):st.write(""" Coeficiente de temperatura-potencia, en""") st.latex(◦C^{−1}). st.write("""Es un parámetro negativo que
refleja cómo incide la temperatura de la celda en el rendimiento del""") st.latex(GFV).st.write(""" Se observa
que incrementos (disminuciones) de""") st.latex(T_c) st.write("""producen, en consecuencia, disminuciones
(incrementos) de""") st.latex(P).
- st.latex(η): st.write("""Rendimiento global de la instalación “por unidad” (valor ideal:""") st.latex(1)st.write("""). Se utiliza para
considerar el efecto de sombras parciales sobre el""") st.latex(GFV)st.write(""", suciedad sobre la superficie
...
st.write("""donde:""")
- st.latex(T_c): st.write("""La Temperatura de la celda en""") st.latex([°C]).
- st.latex(G): st.write("""Irradiancia global incidente en forma normal a los módulos fotovoltaicos, en""") st.latex(W/m^2).
...
de potencia, supere un umbral mínimo""") st.latex(μ)st.write(""", habitualmente expresado en forma porcentual,
en relación a la potencia nominal""") st.latex(P_{inv}) st.write("""del equipo. Si este umbral no es superado, la instalación
no entrega potencia eléctrica. Asimismo, el valor""") st.latex(Pinv) st.write("""(en kilo-Watt) opera como
límite superior del""") st.latex(GFV). st.write("""En consecuencia, la potencia real Pr que entrega la instalación se
puede calcular como:""")
